...
export function AppSidebar({ ...props }: React.ComponentProps<typeof Sidebar>) {
  ...
  useEffect(() => {
    console.log("[CLIENT] AppSidebar component mounted or pathname changed:", pathname)
  }, [pathname])

  const [expandedItems, setExpandedItems] = useState<string[]>(() => {
    const activeParent = dashboardConfig.sidebarNav.find(
      (item) => item.items && item.items.some((subItem) => subItem.href && pathname.startsWith(subItem.href)),
    )
  ...
  return (
    <TooltipProvider>
      ...
      `}</style>
      <Sidebar className={`border-r ${borderColor} transition-colors duration-300`} {...props}>
              ...
              <SidebarMenu className={`space-y-2 transition-all duration-300 ${isCollapsed ? "p-2" : "p-4"}`}>
                {dashboardConfig.sidebarNav.map((item) => {
                  if (item.items) {
                    ...
                    const isParentActive = item.items.some((subItem) => isActive(subItem.href))
